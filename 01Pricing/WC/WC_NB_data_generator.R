...
expected_sev_by_worker_base <- numeric(nrow(nb_data_base))

for (i in seq_len(nrow(nb_data_base))) {
  
  occ <- as.character(nb_data_base$occupation[i])
  ...
  sev_newdata <- nb_data_base[rep(i, nrow(occ_tab)), , drop = FALSE]
...
saveRDS(expected_sev_by_worker_base, file = paste0(getwd(), "/expected_sev_by_worker_base.rds"))